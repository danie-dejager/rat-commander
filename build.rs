//! Packs the vendored binary templates (`assets/templates/*.bt`) into one
//! compressed bundle that `src/bt/bundle.rs` embeds.
//!
//! Format: `b"RCBT0001"`, a u32 entry count, then zlib of the entries, each a
...
fn main() {
    ...
    let dir = Path::new("assets/templates");
    let mut entries: Vec<(String, Vec<u8>)> = std::fs::read_dir(dir)
        .expect("assets/templates")
        .filter_map(|e| e.ok())
        .filter_map(|e| {
            let name = e.file_name().into_string().ok()?;
            if !name.to_ascii_lowercase().ends_with(".bt") {
                return None;
            }
    ...
    z.write_all(&payload).expect("compress templates");
    let compressed = z.finish().expect("compress templates");

    let out = std::path::PathBuf::from(std::env::var_os("OUT_DIR").expect("OUT_DIR"));
    let mut bin = Vec::with_capacity(compressed.len() + 12);
    bin.extend_from_slice(b"RCBT0001");
    bin.extend_from_slice(&(entries.len() as u32).to_le_bytes());
    bin.extend_from_slice(&compressed);
    std::fs::write(out.join("templates.bin"), bin).expect("write templates.bin");
    std::fs::write(
        out.join("templates_meta.rs"),
        format!(
            "/// FNV-1a hash of the bundled templates.\npub const BUNDLE_HASH: u64 = {hash:#018x};\n\
             /// Number of bundled templates.\npub const BUNDLE_COUNT: usize = {};\n",
            entries.len()
        ),
    )
    .expect("write templates_meta.rs");
}
